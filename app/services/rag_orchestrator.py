...
def build_prompt(query, contexts, history=None):
    ...
    if not contexts:
        ...
        return f"""You are a friendly, intelligent AI assistant for the company.
...
Answer:"""
    ...
    for idx, ctx in enumerate(contexts, 1):
        ...
        context_text += ctx["content"].strip() + "\n"
...
def stream_response(query: str, user_role: str, conversation_history=None):
    ...
    cached_response = get_cached_query(query, user_role)
    if cached_response:
        logger.info("Found response in query cache.")
        yield cached_response
        return

    contexts = retrieve(query, user_role, top_k=3)
    ...
    set_cached_query(query, user_role, full_response)